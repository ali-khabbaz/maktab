(function () {
	...
	define(['app'], function (app) {
		...
		homeFactory.$inject = ['$q', '$http', '$location', 'mainViewFactory', '$cacheFactory'];

		function homeFactory(q, http, $location, mvf, $cacheFactory) {
			...
			function getTopCards() {
				var url = mvf.getApiUrl() + 'app/getTopCards',
					dfd = q.defer();
			...
			function getSearchData() {
				var url = mvf.getApiUrl() + 'app/getSearchData',
					dfd = q.defer(),
					temp = [],
					i;
				/*if (cache.get('search')) {
			...
			function getTopCategories() {
				return http.post(mvf.getApiUrl() + 'app/getTopCategories');
			}

			function getArticlesAndSubCategories() {
				return q.all([
					http.post(mvf.getApiUrl() + 'app/getTopSubCategories'),
					http.post(mvf.getApiUrl() + 'app/getTopArticles'),
					http.post(mvf.getApiUrl() + 'app/getBestArticles')
				]);
			}
